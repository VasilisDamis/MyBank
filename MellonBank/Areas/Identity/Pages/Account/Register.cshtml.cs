...
namespace MellonBank.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        private MellonBankUser CreateUser()
        {
            try
            {
                return Activator.CreateInstance<MellonBankUser>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(MellonBankUser)}'. " +
                    $"Ensure that '{nameof(MellonBankUser)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<MellonBankUser> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<MellonBankUser>)_userStore;
        }
    }
}
